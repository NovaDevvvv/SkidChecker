...
export function scanSources(files) {
  ...
  for (const signature of SIGNATURES) {
    const match = findMatch(files, signature);
    ...
    checks.push({
      ...
      evidence: match ? buildSnippet(match.content, signature) : null
    });
    ...
    detections.push({
      ...
      snippet: buildSnippet(match.content, signature)
    });
  }
  ...
  const score = Math.min(
    100,
    detections.reduce((total, entry) => {
      const severity = severityWeight[entry.severity] ?? 0;
      return total + Math.round(severity * (entry.confidence / 100));
    }, 0)
  );
...
function findMatch(files, signature) {
...
function buildSnippet(content, signature) {
  if (signature.id === 'weedhack-payload') {
    const marker = 'minecraftInfo';
  ...
  if (signature.id === 'minecraft-session-token-access') {
  ...
  const genericNeedle = /method_1674|accessToken|launcher_profiles\.json|accounts\.json|webhooks|Runtime\.getRuntime\(\)\.exec|powershell/i.exec(content);
  if (genericNeedle?.index != null) {
    return crop(content, genericNeedle.index, 260);
  }
